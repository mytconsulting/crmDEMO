  ...
  return (
    <div className="wm-backdrop" role="dialog" aria-modal="true" aria-label="Bienvenido a SmartFunnel">
      ...
      <div className="wm-card">
        ...
        <p className="wm-text">
          Somos <strong>M&T Consulting</strong>. SmartFunnel es nuestro <strong>CRM con IA</strong> para
          PYMEs: capta, atiende y hace seguimiento a tus leads por WhatsApp e Instagram, y agenda las citas
          por ti.
        </p>
        <p className="wm-text wm-text--muted">
          Es una <strong>demo con datos de ejemplo</strong> — explora con total libertad.
        </p>
...
const wmCss = `
.wm-backdrop { position: fixed; inset: 0; z-index: 12000; display: flex; padding: 24px; background: rgba(11,15,20,.55); backdrop-filter: blur(4px); animation: wmFade .2s ease-out; overflow-y: auto; }
...
.wm-card { position: relative; width: min(540px, 100%); margin: auto; background: var(--surface, #fff); border: 1px solid var(--border, #e4e6eb); border-radius: 20px; padding: 26px 30px 20px; box-shadow: 0 30px 70px rgba(11,18,32,.28); animation: wmPop .28s cubic-bezier(.2,.8,.2,1); }
@media (max-width: 560px) { .wm-card { padding: 22px 18px 18px; border-radius: 16px; } }
...
.wm-brand { display: flex; align-items: center; gap: 11px; margin-bottom: 14px; }
...
.wm-title { font-size: 22px; letter-spacing: -0.03em; line-height: 1.12; margin-bottom: 8px; color: var(--ink, #0b0f14); }
.wm-text { font-size: 14px; line-height: 1.55; color: var(--slate, #4a5468); margin-bottom: 7px; }
.wm-text--muted { color: var(--slate-2, #7a8599); font-size: 13px; }

.wm-form { margin: 16px 0 14px; padding: 16px; background: var(--surface-2, #f4f5f7); border: 1px solid var(--border, #e4e6eb); border-radius: 14px; }
.wm-form__head { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
...
.wm-input { height: 38px; padding: 0 12px; background: var(--surface, #fff); border: 1px solid var(--border-strong, #c9d0db); border-radius: 9px; font-family: inherit; font-size: 13.5px; color: var(--ink, #0b0f14); outline: none; transition: border-color .12s, box-shadow .12s; }
...
.wm-cta { width: 100%; height: 46px; display: inline-flex; align-items: center; justify-content: center; gap: 8px; border: none; border-radius: 11px; background: var(--tide, #16D998); color: var(--ink, #0b0f14); font-family: inherit; font-size: 14.5px; font-weight: 700; letter-spacing: -0.01em; cursor: pointer; transition: background .15s, transform .1s; }
.wm-cta:hover { background: #12C58C; }
.wm-cta:active { transform: translateY(1px); }
.wm-cta:disabled { opacity: .7; cursor: default; }

.wm-contact { margin-top: 14px; padding-top: 12px; border-top: 1px solid var(--border, #e4e6eb); text-align: center; }
.wm-contact__label { display: block; font-size: 12.5px; color: var(--slate-2, #7a8599); margin-bottom: 6px; }
